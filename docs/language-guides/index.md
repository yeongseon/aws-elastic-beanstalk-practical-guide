...
This section provides step-by-step tutorials for deploying and operating web applications on AWS Elastic Beanstalk using two language tracks: **Python (Flask)** and **Node.js (Express)**.
...
Both language tracks share the same seven-step structure, followed by targeted recipes for common integration patterns.
...
| Factor | Python (Flask) | Node.js (Express) |
|---|---|---|
| WSGI/ASGI server | Gunicorn via Procfile | Node.js built-in HTTP |
| Reverse proxy | nginx (default) | nginx (default) |
| Dependency file | `requirements.txt` | `package.json` |
| Process definition | `Procfile` | `Procfile` or `npm start` |
| Platform hooks | Bash scripts in `.platform/hooks/` | Bash scripts in `.platform/hooks/` |
| Static files | Typically served via S3 or nginx | Typically served via Express static or nginx |

Both tracks use identical Elastic Beanstalk features. The choice depends on your team's language preference and existing application stack.
