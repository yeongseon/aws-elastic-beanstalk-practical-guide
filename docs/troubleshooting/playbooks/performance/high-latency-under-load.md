...
## 1. Summary
Response times rise sharply as traffic increases even though the environment looks healthy at low concurrency.

```mermaid
flowchart TD
    A[High Latency Under Load] --> B{Primary branch}
    B --> C1[Insufficient serving capacity]
    C1 --> D1[Collect logs, metrics, and platform signals]
    B --> C2[Slow database or downstream dependency]
    C2 --> D2[Collect logs, metrics, and platform signals]
    B --> C3[Connection pool exhaustion]
    C3 --> D3[Collect logs, metrics, and platform signals]
    B --> C4[Worker or instance sizing mismatch]
...
- - H1: Insufficient serving capacity — Primary evidence should confirm or disprove whether insufficient serving capacity.
- - H2: Slow database or downstream dependency — Primary evidence should confirm or disprove whether slow database or downstream dependency.
- - H3: Connection pool exhaustion — Primary evidence should confirm or disprove whether connection pool exhaustion.
- - H4: Worker or instance sizing mismatch — Primary evidence should confirm or disprove whether worker or instance sizing mismatch.
...
| filter @message like /rt=|slow query/
| sort @timestamp asc
| limit 20
```
...
| @timestamp | @message |
| --- | --- |
| 2026-04-07 09:15:06 | rt= |
| 2026-04-07 09:15:17 | slow query |
...
```sql
fields @timestamp, @message
| filter @message like /pool timeout|timed out/
| sort @timestamp desc
| limit 20
```
...
| @timestamp | @message |
| --- | --- |
| 2026-04-07 09:15:21 | pool timeout |
...
### H1: Insufficient serving capacity

Confirm:
- Logs, metrics, and platform state all point directly at this branch.
- The first failing timestamp lines up with evidence expected for `Insufficient serving capacity`.
...
### H2: Slow database or downstream dependency

Confirm:
- Logs, metrics, and platform state all point directly at this branch.
- The first failing timestamp lines up with evidence expected for `Slow database or downstream dependency`.
...
Confirm:
- Logs, metrics, and platform state all point directly at this branch.
- The first failing timestamp lines up with evidence expected for `Connection pool exhaustion`.

Disprove:
- The expected log or state change for this branch never appears.
- Another branch has earlier, stronger, and more direct evidence.

### H4: Worker or instance sizing mismatch

Confirm:
- Logs, metrics, and platform state all point directly at this branch.
- The first failing timestamp lines up with evidence expected for `Worker or instance sizing mismatch`.
