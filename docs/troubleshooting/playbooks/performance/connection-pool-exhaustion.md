...
## 1. Summary
Requests slow down or fail because application workers spend time waiting for database or outbound HTTP connections rather than doing useful work.

```mermaid
flowchart TD
    A[Connection Pool Exhaustion] --> B{Primary branch}
    B --> C1[Pool limit is too small for traffic]
    C1 --> D1[Collect logs, metrics, and platform signals]
    B --> C2[Connections are leaked or held too long]
    C2 --> D2[Collect logs, metrics, and platform signals]
    B --> C3[Downstream service is slow]
    C3 --> D3[Collect logs, metrics, and platform signals]
    B --> C4[Worker concurrency exceeds connection budget]
...
- - H1: Pool limit is too small for traffic — Primary evidence should confirm or disprove whether pool limit is too small for traffic.
- - H2: Connections are leaked or held too long — Primary evidence should confirm or disprove whether connections are leaked or held too long.
- - H3: Downstream service is slow — Primary evidence should confirm or disprove whether downstream service is slow.
- - H4: Worker concurrency exceeds connection budget — Primary evidence should confirm or disprove whether worker concurrency exceeds connection budget.
...
| filter @message like /connection pool timeout|waiting for idle client/
| sort @timestamp asc
| limit 20
```
...
| @timestamp | @message |
| --- | --- |
| 2026-04-07 09:15:06 | connection pool timeout |
| 2026-04-07 09:15:17 | waiting for idle client |
...
```sql
fields @timestamp, @message
| filter @message like /lock wait|transaction open/
| sort @timestamp desc
| limit 20
```
...
| @timestamp | @message |
| --- | --- |
| 2026-04-07 09:15:21 | lock wait |
| 2026-04-07 09:15:28 | transaction open |
...
### H1: Pool limit is too small for traffic

Confirm:
- Logs, metrics, and platform state all point directly at this branch.
- The first failing timestamp lines up with evidence expected for `Pool limit is too small for traffic`.
...
### H2: Connections are leaked or held too long

Confirm:
- Logs, metrics, and platform state all point directly at this branch.
- The first failing timestamp lines up with evidence expected for `Connections are leaked or held too long`.

Disprove:
- The expected log or state change for this branch never appears.
- Another branch has earlier, stronger, and more direct evidence.

### H3: Downstream service is slow

Confirm:
- Logs, metrics, and platform state all point directly at this branch.
- The first failing timestamp lines up with evidence expected for `Downstream service is slow`.

Disprove:
- The expected log or state change for this branch never appears.
- Another branch has earlier, stronger, and more direct evidence.

### H4: Worker concurrency exceeds connection budget

Confirm:
- Logs, metrics, and platform state all point directly at this branch.
- The first failing timestamp lines up with evidence expected for `Worker concurrency exceeds connection budget`.
