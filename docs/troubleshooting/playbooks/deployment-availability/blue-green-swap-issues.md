# Blue/Green Swap Issues

## 1. Summary
A CNAME swap or blue/green cutover completes but traffic, health, or configuration behavior is not what operators expect.

```mermaid
flowchart TD
    A[Blue/Green Swap Issues] --> B{Primary branch}
    B --> C1[DNS or client caching confusion]
    C1 --> D1[Collect logs, metrics, and platform signals]
    B --> C2[Green environment not truly production-ready]
    C2 --> D2[Collect logs, metrics, and platform signals]
    B --> C3[Configuration drift between blue and green]
    C3 --> D3[Collect logs, metrics, and platform signals]
    B --> C4[Asynchronous dependencies still point at blue]
...
- - H1: DNS or client caching confusion — Primary evidence should confirm or disprove whether dns or client caching confusion.
- - H2: Green environment not truly production-ready — Primary evidence should confirm or disprove whether green environment not truly production-ready.
- - H3: Configuration drift between blue and green — Primary evidence should confirm or disprove whether configuration drift between blue and green.
- - H4: Asynchronous dependencies still point at blue — Primary evidence should confirm or disprove whether asynchronous dependencies still point at blue.
...
| filter @message like /host=www.example.com|env=green-env/
| sort @timestamp asc
| limit 20
```
...
| @timestamp | @message |
| --- | --- |
| 2026-04-07 09:15:06 | host=www.example.com |
| 2026-04-07 09:15:17 | env=green-env |
...
```sql
fields @timestamp, @message
| filter @message like /Missing environment variable|webhook/
| sort @timestamp desc
| limit 20
```
...
| @timestamp | @message |
| --- | --- |
| 2026-04-07 09:15:21 | Missing environment variable |
| 2026-04-07 09:15:28 | webhook |
...
### H1: DNS or client caching confusion

Confirm:
- Logs, metrics, and platform state all point directly at this branch.
- The first failing timestamp lines up with evidence expected for `DNS or client caching confusion`.
...
### H2: Green environment not truly production-ready

Confirm:
- Logs, metrics, and platform state all point directly at this branch.
- The first failing timestamp lines up with evidence expected for `Green environment not truly production-ready`.

Disprove:
- The expected log or state change for this branch never appears.
- Another branch has earlier, stronger, and more direct evidence.

### H3: Configuration drift between blue and green

Confirm:
- Logs, metrics, and platform state all point directly at this branch.
- The first failing timestamp lines up with evidence expected for `Configuration drift between blue and green`.

Disprove:
- The expected log or state change for this branch never appears.
- Another branch has earlier, stronger, and more direct evidence.

### H4: Asynchronous dependencies still point at blue

Confirm:
- Logs, metrics, and platform state all point directly at this branch.
- The first failing timestamp lines up with evidence expected for `Asynchronous dependencies still point at blue`.
