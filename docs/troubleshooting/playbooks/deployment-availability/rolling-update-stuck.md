...
## 1. Summary
A Rolling or Rolling with Additional Batch deployment remains in `Updating` far longer than expected and does not advance to the next batch.

```mermaid
flowchart TD
    A[Rolling Update Stuck] --> B{Primary branch}
    B --> C1[Long-running deployment steps]
    C1 --> D1[Collect logs, metrics, and platform signals]
    B --> C2[Health flapping blocks batch advancement]
    C2 --> D2[Collect logs, metrics, and platform signals]
    B --> C3[Desired capacity cannot settle]
    C3 --> D3[Collect logs, metrics, and platform signals]
    B --> C4[One replacement instance is persistently unhealthy]
...
- - H1: Long-running deployment steps — Primary evidence should confirm or disprove whether long-running deployment steps.
- - H2: Health flapping blocks batch advancement — Primary evidence should confirm or disprove whether health flapping blocks batch advancement.
- - H3: Desired capacity cannot settle — Primary evidence should confirm or disprove whether desired capacity cannot settle.
- - H4: One replacement instance is persistently unhealthy — Primary evidence should confirm or disprove whether one replacement instance is persistently unhealthy.
...
| filter @message like /Waiting for instances|batch/
| sort @timestamp asc
| limit 20
```
...
| @timestamp | @message |
| --- | --- |
| 2026-04-07 09:15:06 | Waiting for instances |
| 2026-04-07 09:15:17 | batch |
...
```sql
fields @timestamp, @message
| filter @message like /migration|timed out/
| sort @timestamp desc
| limit 20
```
...
### H1: Long-running deployment steps

Confirm:
- Logs, metrics, and platform state all point directly at this branch.
- The first failing timestamp lines up with evidence expected for `Long-running deployment steps`.
...
### H2: Health flapping blocks batch advancement

Confirm:
- Logs, metrics, and platform state all point directly at this branch.
- The first failing timestamp lines up with evidence expected for `Health flapping blocks batch advancement`.

Disprove:
- The expected log or state change for this branch never appears.
- Another branch has earlier, stronger, and more direct evidence.

### H3: Desired capacity cannot settle

Confirm:
- Logs, metrics, and platform state all point directly at this branch.
- The first failing timestamp lines up with evidence expected for `Desired capacity cannot settle`.

Disprove:
- The expected log or state change for this branch never appears.
- Another branch has earlier, stronger, and more direct evidence.

### H4: One replacement instance is persistently unhealthy

Confirm:
- Logs, metrics, and platform state all point directly at this branch.
- The first failing timestamp lines up with evidence expected for `One replacement instance is persistently unhealthy`.
